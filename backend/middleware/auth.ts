...
export const authenticateToken = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  // ИСПРАВЛЕНО: Пропускаем OPTIONS запросы (preflight) без проверки токена
  // Браузер отправляет OPTIONS запросы для CORS preflight проверки без заголовков авторизации
