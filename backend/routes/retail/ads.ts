...
router.post('/ads/:id/images', upload.array('images', 10), async (req: any, res: any) => {
  try {
    const { id } = req.params;
...
// Удалить изображение
router.delete('/ads/:id/images/:imageId', async (req: any, res: any) => {
  try {
...
router.put('/ads/:id/images/order', async (req: any, res: any) => {
  try {
    const { id } = req.params;
...
router.post('/ads/:id/request', authenticateToken, createAdRequest);

// Получить запросы для объявления (для создателя)
router.get('/ads/:id/requests', authenticateToken, getAdRequests);

// Получить запросы пользователя (для того, кто запрашивает)
router.get('/ads/requests/my', authenticateToken, getUserAdRequests);

// Подтвердить запрос
router.post('/ads/requests/:requestId/approve', authenticateToken, approveAdRequest);

// Отклонить запрос
router.post('/ads/requests/:requestId/reject', authenticateToken, rejectAdRequest);

// Добавить номер документа отгрузки
router.post('/ads/requests/:requestId/shipment-doc', authenticateToken, addShipmentDocNumber);
